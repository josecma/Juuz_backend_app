...
> {
  ...
  async createSend(
    ...
    const privateUserChannel = await this.getPrivateUserChannelByUserIdUseCase.execute({ userId: data.driverId.toString() });

    const user = await this.findOneUserByIdService.find({ id: driverId.toString() });

    const company = await this.findCompanyByOwnerIdService.find(driverId.toString());
